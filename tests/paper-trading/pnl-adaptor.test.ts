...
import type { ClosedTrade } from "../../src/paper-trading/trade-analyst.js";
...
describe("decideSizeMultiplier", () => {
  it("returns null below the minimum sample size", () => {
    ...
    expect(decideSizeMultiplier(trades, 2, 1, cfg)).toBeNull();
  });
  ...
  it("scales down when live PF trails backtest PF (below 70%)", () => {
    ...
    const result = decideSizeMultiplier(trades, 2, 1, cfg);
  ...
  it("scales up when live PF clears backtest PF outright", () => {
    const trades = tradesWithPf(cfg.minSampleSize, 3); // LivePf > backtestPf=1.5
  ...
  it("caps the scale-up at sizeMultiplierMax over repeated cycles", () => {
    const trades = tradesWithPf(cfg.minSampleSize, 3);
...
describe("decidePrune", () => {
  ...
  it("does not prune when the most recent slice has recovered above floor", () => {
    ...
    const recovered = tradesWithPf(cfg.recoverySliceSize, 5); // Strong recent PF pulls the recovery slice up
