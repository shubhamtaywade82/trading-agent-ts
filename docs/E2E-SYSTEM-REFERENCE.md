...
- Fill model: `feeBps` (5) charged on both entry and exit notionals (`(entryNotional + exitNotional) * feeFrac`), `slippageBps` (3) applied against entry price in the adverse direction, simulated liquidation price computed via exact Binance USDT-M Isolated Margin formula ($P_{\text{liq}} = P_{\text{entry}} \times \frac{1 \mp 1/L}{1 \mp \text{MMR}}$ with MMR default 0.5%).
...
- Pool hot-reload (`reloadPool`): picks up strategies newly appended to `strategies.json` (e.g. by the auto-research pipeline) without a process restart. Also picks up, for **existing** ids only (added 2026-07-19), a changed `sizeMultiplier` or an `enabled:false` prune written by `PnlAdaptor` (§5.5) — never mutates `entry`/`risk`/`maxHoldBars`/`direction` in place. A pruned id is simply dropped from the active pool; its open position (if any) still exits normally since exits are position-driven, not strategy-list-driven.
- `StrategyDef.sizeMultiplier` (default 1, added 2026-07-19): a per-strategy sizing multiplier applied to entry qty alongside the AI gate's own multiplier (composed, not replaced) — the mechanism `PnlAdaptor` (§5.5) uses to resize an existing strategy based on live PnL.
...
| `drift-monitor.ts` (`DriftMonitor`) | Proactively compares live win-rate/PF against the strategy's backtested reference, alerts once when a threshold is first crossed (re-arms if it recovers then drifts again). **Basis-drift alert** (added 2026-07-17): also reads `coindcx-basis.jsonl` (§5.4/§5.7) independent of the trade-count throttle (basis data accumulates on every fill, faster than closed trades) — per symbol, once at least `basisWindow` (20) fills exist, alerts if the average `|basisBps|` over the last `basisWindow` fills exceeds `basisThresholdBps` (15), same re-arming pattern as the WR/PF alerts. Covered by `tests/paper-trading/drift-monitor.test.ts`. |
...
| `research-pipeline.ts` (`ResearchPipeline`) | Autonomously **adds** new strategies to `strategies.json`. Weekly (configurable), sweeps the existing validated signal-type pool across new (symbol, timeframe, stop/target) combinations using the same `runFuturesBacktest` engine, requires a 3-fold contiguous-window OOS pass (every fold independently profitable) before promoting, caps promotions per cycle (3) and total pool size (40). New signal *types* are never invented here — only re-combinations of already-validated condition types. Promoted strategies start with their own fresh isolated capital bucket and get hot-reloaded into the running `LivePaperRunner` without restart. |
| `pnl-adaptor.ts` (`PnlAdaptor`, added 2026-07-19) | The one place the system **mutates or removes an existing strategy** based on live results — `ResearchPipeline` above only adds. Hourly: `decideSizeMultiplier` nudges an existing strategy's `sizeMultiplier` up/down (bounded `±0.1`/cycle, clamped `[0.25, 1.2]`) when live PF (last 30+ trades) diverges from its own backtest PF; `decidePrune` soft-disables (`enabled:false`) a strategy whose live PF stays decisively under `0.5` over 60+ trades with no recovery in the most recent 15. Both are pure functions, independently unit-tested (`tests/paper-trading/pnl-adaptor.test.ts`). **Ships dry-run by default** (`TRADINGAGENT_PNL_ADAPTOR_LIVE=true` to opt into writing `strategies.json`/calling `reloadPool` for real) — every decision, live or dry-run, is logged to `.trading-agent/pnl-adjustments.jsonl`. Deliberately stricter/longer-horizon than `StrategyCircuitBreaker`'s temporary pause below — this is a permanent-until-a-human-reverts action. |
| `ai-gate.ts` (`AiEntryGate`) | Opt-in (`RunnerConfig.aiMode`, default `no-ai`) pre-trade LLM veto/sizer — a deliberate, documented reversal of `TradeAnalyst`/`TradeEvaluator`'s read-only boundary. Fails closed on any timeout/error/unparseable response. As of 2026-07-19, checks the model's real tool-calling capability (via a `ModelCatalog`-shaped `CapabilityLookup`, local Ollama's `/api/tags` capabilities array being authoritative) and, if supported, issues a single structured `submit_decision` tool call instead of regex-parsing free text; falls back to the original regex-parsed prompt (not fail-closed) if tool support is absent or unverified. Default model: `minicpm5-1b` (local tier) / `gpt-oss:20b` (cloud tier, the live `strategies.json` default) — see §7.1. |
...
- `PnlAdaptor` (hourly, §5.5) — dry-run unless `TRADINGAGENT_PNL_ADAPTOR_LIVE=true`
...
| `pnl-adaptor-state.json` | PnlAdaptor | Per-strategy last-adjusted trade-count, so unchanged data isn't re-adjusted every cycle |
| `pnl-adjustments.jsonl` | PnlAdaptor | Every resize/prune decision, live or dry-run (`dryRun` field distinguishes) |
...
| `coindcx-basis.jsonl` | LivePaperRunner (via `coindcx-shadow.ts`); read by DriftMonitor | Per-fill Binance-vs-CoinDCX basis in bps — best-effort write, added 2026-07-17; read back for the basis-drift alert, added same day |
...
- Binance vs. CoinDCX gap: liquidity, spread, and available symbols differ between exchanges. Paper results were generated against **Binance** prices; the eventual live-execution target is **CoinDCX** (§1a). Expect some drift between paper and live performance from this alone, independent of code correctness. As of 2026-07-17 this is instrumented and alerted on — `coindcx-shadow.ts` logs the real Binance-vs-CoinDCX basis on every paper fill, and `DriftMonitor` now alerts per symbol when the rolling average basis exceeds a threshold (§5.5). Still open: nothing yet adjusts strategy metrics or the readiness gate for basis — the alert is informational, a human still decides what to do about it.
...
| `TRADINGAGENT_ANALYST_TIER` | `cloud` | LLM tier for TradeAnalyst/TradeEvaluator/ChatAssistant/AiEntryGate |
| `TRADINGAGENT_ANALYST_MODEL` | `gpt-oss:20b` (cloud) / `minicpm5-1b` (local) | Model for advisory LLM components, incl. AiEntryGate |
| `TRADINGAGENT_PNL_ADAPTOR_LIVE` | unset (dry-run) | Set to `true` to let `PnlAdaptor` (§5.5) actually write `strategies.json` / call `reloadPool` instead of only logging what it would do |
...
- **2026-07-17** — Added the basis-drift alert to `DriftMonitor` (`src/paper-trading/drift-monitor.ts`), the one concrete follow-up from an external review of the above: `coindcx-basis.jsonl` was being written but nothing read it back. New `readBasisRecords()` in `coindcx-shadow.ts`, new `checkBasisDrift()` in `DriftMonitor` — per symbol, once `basisWindow` (20) fills exist, alerts once when the rolling average `|basisBps|` crosses `basisThresholdBps` (15), same re-arming-flag pattern as the existing WR/PF alerts, runs independent of the trade-count throttle since basis records accumulate on every fill (entry + exit), faster than closed trades. `DriftState` gained an `alertedBasis` map with a back-compat default for state files written before this change. Declined from the same review round: a minimum-time (30-day) readiness criterion, exit-side slippage modeling, and CoinDCX futures-vs-spot ticker — all correctly flagged by the reviewer as non-blockers, left for later. New tests: `tests/paper-trading/drift-monitor.test.ts` (5 tests — window-minimum gating, first alert, no-repeat-alert while still drifting, re-arm after recovery, stays quiet under threshold).
- **2026-07-19** — Two additions prompted by a user question about whether the system is "self-learning/autonomous" and whether it uses MiniCPM5-1B for intent identification; answer at the time was "partial" and "only if tier=local" respectively — this closes both gaps:
  - **`AiEntryGate` structured tool-calling** (§5.5) — replaced regex-parsed free text (`decision: APPROVE size=1.0`) with a single structured `submit_decision` tool call when the configured model's tool-calling capability is verified via `ModelCatalog`/`CapabilityLookup` (local Ollama's `/api/tags` capabilities array is authoritative; cloud-tier detection is an unverified name heuristic, logged once if relied on). Falls back to the original regex path — not fail-closed — when tool support is absent/unverified, since the gate's fail-closed guarantee is about ambiguous/erroring responses, not tool-call availability. `ChatCapable.chat()` widened to accept `ChatOptions` (2nd arg, already supported by `Provider`/`chat-assistant.ts`, just unused here before). New tests: 8 added to `tests/paper-trading/ai-gate.test.ts` (13 total).
  - **`PnlAdaptor`** (§5.5, new `src/paper-trading/pnl-adaptor.ts`) — the live-PnL feedback loop that was missing: `ResearchPipeline` only ever added strategies, `StrategyCircuitBreaker` only ever paused temporarily; nothing adjusted an *existing* strategy's sizing or permanently retired a decisive underperformer based on live results. Adds `StrategyDef.sizeMultiplier` (composed with the AI gate's own multiplier) and reads/writes an `enabled` flag in `strategies.json`; `LivePaperRunner.reloadPool` extended to pick up both for existing ids (previously strictly add-only — that invariant is now scoped to `entry`/`risk`/`maxHoldBars`/`direction`, never mutated in place). **Ships dry-run by default**, gated live by `TRADINGAGENT_PNL_ADAPTOR_LIVE=true`. New tests: `tests/paper-trading/pnl-adaptor.test.ts` (9, pure `decideSizeMultiplier`/`decidePrune` functions), `tests/paper-trading/live-runner-reload.test.ts` (4, mutate-in-place/prune behavior on a real `LivePaperRunner` instance against tmpdir fixtures). Full test suite (72 suites / 520 tests) green after both changes, `tsc --noEmit` clean.
