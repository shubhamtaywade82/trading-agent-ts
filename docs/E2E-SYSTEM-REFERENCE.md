...
**No live order execution exists anywhere in the codebase.** No exchange API keys, no HMAC-signed requests, no order-placement calls. The user's stated plan is to wire live execution to **CoinDCX** separately; this repo currently produces the signal/risk layer that a future execution layer would consume.
...
**Risk controls added in this session (2026-07-17), verify against current file if reviewing later:**
...
- `LivePaperRunner` + `BinanceStreamManager` (event-driven entries, REST poll as safety net)
...
- `StrategyCircuitBreaker`, `DriftMonitor` (5min interval each)
...
- Binance vs. CoinDCX gap: liquidity, spread, and available symbols differ between exchanges. Paper results were generated against **Binance** prices; the user's live-execution target is **CoinDCX**. Expect some drift between paper and live performance from this alone, independent of code correctness — the drift monitor exists but currently only compares live-paper vs. backtest, not live-paper vs. a different exchange's fill quality.
